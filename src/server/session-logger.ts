...
export class SessionLogger {
  ...
  private streams: Map<SessionId, WriteStream> = new Map();
   ...
   */
  private getStream(sessionId: SessionId): WriteStream {
  ...
  writeMessage(sessionId: SessionId, message: AnyMessage): void {
    try {
      ...
      logger.warn({ error: (err as Error).message, sessionId, messageId: message.id }, "Failed to write session log");
    }
  }
