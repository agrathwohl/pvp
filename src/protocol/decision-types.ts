...
export type GitBranchRef = string & { readonly __brand: 'GitBranchRef' };

/** Git tag name (refs/tags/...) */
export type GitTagRef = string & { readonly __brand: 'GitTagRef' };

/** Decision commit identifier - unique within a repository */
export type DecisionCommitId = string & { readonly __brand: 'DecisionCommitId' };

/** Decision tree identifier - links PVP session to git history */
export type DecisionTreeId = string & { readonly __brand: 'DecisionTreeId' };

/** Conversation thread identifier */
export type ConversationThreadId = string & { readonly __brand: 'ConversationThreadId' };
...
export type DecisionType =
  ...
  | 'exploration'     // Experimental or investigative changes
  | 'revert'          // Undoing previous changes
  | 'documentation'   // Documentation-only changes
  | 'test'            // Test additions or modifications
  | 'configuration'   // Config/build system changes
...
// =============================================================================
// SUPPORTING TYPES FOR DECISION COMMIT
// =============================================================================
...
export type CompactMessageType =
  | 'prompt'      // Human prompt
  | 'response'    // AI response
  | 'thinking'    // AI thinking (optional inclusion)
  | 'tool'        // Tool proposal/execution/result
  | 'approval'    // Gate/tool approval
  | 'rejection'   // Gate/tool rejection
  | 'interrupt'   // Interrupt
  | 'context'     // Context change
  | 'system';     // System messages (fork, merge, etc.)
...
export interface GitTrailers {
  ...
  'PVP-Confidence': string;  // Stringified number
  'PVP-Type': DecisionType;
  'PVP-Thread'?: string;     // Base64-encoded compact thread
  'PVP-Tools'?: string;      // Comma-separated tool list
  'PVP-Approvers'?: string;  // Comma-separated approver list
}
...
export function isGitSha(value: string): value is GitSha {
  return /^[a-f0-9]{40}$/.test(value);
}

/** Validate a string is a valid GitBranchRef */
export function isGitBranchRef(value: string): value is GitBranchRef {
...
export function generateDecisionCommitId(): DecisionCommitId {
  return `dec_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}` as DecisionCommitId;
}

/** Generate a ConversationThreadId */
export function generateConversationThreadId(): ConversationThreadId {
  return `thr_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}` as ConversationThreadId;
}

/** Generate a DecisionTreeId */
export function generateDecisionTreeId(): DecisionTreeId {
  return `tree_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}` as DecisionTreeId;
}
