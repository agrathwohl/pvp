...
export class ClaudeAgent {
  ...
  private tasksToolHandler: ReturnType<typeof createTasksToolHandler>;
  private tasksProposals: Map<MessageId, { args: TaskOperationArgs; toolUseId: string }> = new Map();
  private monitorToolHandler: ReturnType<typeof createMonitorToolHandler>;
  private monitorProposals: Map<MessageId, { input: ProcessMonitorInput; toolUseId: string }> = new Map();
          ...
          if (message.payload.key === TASKS_CONTEXT_KEY) {
            try {
              ...
              if (content && typeof content === "object") {
                const tasksState = content as SerializedTasksState;
  ...
  public async proposeMonitor(
    ...
    try {
      const proposalMsg = this.monitorToolHandler.proposeMonitor(
        input,
        this.sessionId,
        this.participantId
      );
   ...
   */
  private async executeMonitor(proposalId: MessageId): Promise<void> {
    if (!this.sessionId) return;
    ...
    try {
      ...
      await this.monitorToolHandler.executeMonitor(
        proposalId,
        input,
        this.sessionId,
        this.participantId,
   ...
   */
  public validateAgainstTasks(): { valid: boolean; reason: string } {
   ...
   */
  public getStrictModeContext(): string | null {
   ...
   */
  public async proposeTasksOperation(
    ...
    try {
      const proposalMsg = this.tasksToolHandler.proposeTasksOperation(
        args,
        this.sessionId,
        this.participantId
      );
   ...
   */
  private async executeTasks(proposalId: MessageId): Promise<void> {
    if (!this.sessionId) return;
    ...
    try {
      ...
      await this.tasksToolHandler.executeTasksOperation(
        proposalId,
        args,
        this.sessionId,
        this.participantId,
        ...
        if (toolUse.name === "execute_shell_command") {
          ...
          if (!input.command) {
            ...
            this.markToolFailed(toolUse.id,
              `TOOL VALIDATION FAILED: execute_shell_command is missing the required "command" field. ` +
          ...
          if (!input.path || input.content === undefined) {
            const missingFields = [];
            if (!input.path) missingFields.push("path");
            if (input.content === undefined) missingFields.push("content");
            ...
            this.markToolFailed(toolUse.id,
              `TOOL VALIDATION FAILED: file_write is missing required field(s): ${missingFields.join(", ")}. ` +
          ...
          if (!input.path || input.old_text === undefined || input.new_text === undefined) {
            const missingFields = [];
            if (!input.path) missingFields.push("path");
            if (input.old_text === undefined) missingFields.push("old_text");
            if (input.new_text === undefined) missingFields.push("new_text");
            logger.error({ toolUseId: toolUse.id, missingFields }, "File edit input missing required fields");
            this.markToolFailed(toolUse.id,
              `TOOL VALIDATION FAILED: file_edit is missing required field(s): ${missingFields.join(", ")}. ` +
          ...
          if (!input.type || !input.description) {
            ...
            this.markToolFailed(toolUse.id,
              `TOOL VALIDATION FAILED: git_commit is missing required field(s): ${missingFields.join(", ")}. ` +
          ...
          if (!input.notebook_path) {
            ...
            this.markToolFailed(toolUse.id,
              `TOOL VALIDATION FAILED: notebook_execute is missing the required "notebook_path" field. ` +
          ...
          const input = toolUse.input as TaskOperationArgs;
          if (!input.operation) {
          ...
          const input = toolUse.input as unknown as ProcessMonitorInput;
          if (!input.sources || input.sources.length === 0) {
            logger.error({ toolUseId: toolUse.id }, "Process monitor input missing sources");
            this.markToolFailed(toolUse.id,
              `TOOL VALIDATION FAILED: process_monitor requires at least one data source. ` +
          ...
          if (!input.stop_condition) {
            ...
            this.markToolFailed(toolUse.id,
              `TOOL VALIDATION FAILED: process_monitor requires a stop_condition. ` +
