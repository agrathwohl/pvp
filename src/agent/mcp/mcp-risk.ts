...
const CATEGORY_PRIORITY: ToolCategory[] = [
  "secret_access",
  "deploy",
  "shell_execute",
  "file_delete",
...
const CATEGORY_RISK_LEVELS: Record<ToolCategory, RiskLevel> = {
  file_read: "low",
  file_write: "medium",
  file_delete: "high",
  shell_execute: "high",
...
const TRUST_LEVEL_APPROVAL: Record<MCPServerConfig["trust_level"], boolean> = {
  untrusted: true,
  low: true,
...
export function categorizeMCPTool(
  ...
  // Determine category
  let category: ToolCategory = serverConfig.default_category;
  ...
  let risk_level: RiskLevel = override?.risk_level ?? CATEGORY_RISK_LEVELS[category];
...
export function isToolBlocked(toolName: string, serverConfig: MCPServerConfig): {
  blocked: boolean;
  reason?: string;
} {
