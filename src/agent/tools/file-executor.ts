...
// Risk levels matching shell-executor pattern
...
export interface FileEditCommand {
  ...
  oldText: string;
  newText: string;
  occurrence: number; // 0 = all, 1 = first, 2 = second, etc.
  category: FileCategory;
  riskLevel: RiskLevel;
  requiresApproval: boolean;
}
 ...
 */
const PATH_PATTERNS: FilePathPattern[] = [
...
export function categorizeFilePath(
  ...
  return {
    ...
    riskLevel,
    requiresApproval,
  } as FileWriteCommand & FileEditCommand;
}
 ...
 */
export function getDefaultFileConfig(): { maxFileSize: number; encoding: BufferEncoding } {
  return {
