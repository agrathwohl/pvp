...
class PVPServer {
     ...
     */
    async autoCreateSession(sessionId, participantId) {
        const workingDirectory = await this.initializeSessionDirectory(sessionId);
    ...
    handleDisconnect(participantId) {
        ...
        for (const session of this.sessions.values()) {
            ...
            if (participant) {
                ...
                const leaveMsg = createMessage("session.leave", session.getId(), participantId, {
                    reason: "Client disconnected",
                });
                this.transportServer.broadcast(leaveMsg);
                // Also send presence update
                const presenceMsg = createMessage("presence.update", session.getId(), "system", {
    ...
    startHeartbeatMonitoring(session, participantId) {
        ...
        const interval = setInterval(() => {
            ...
            const pingMsg = createMessage("heartbeat.ping", session.getId(), "system", {});
     ...
     */
    updateBridgeParticipants(session) {
