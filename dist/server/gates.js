...
export class GateManager {
    evaluateQuorum(gate, participants) {
        ...
        switch (quorum.type) {
            ...
            case "all": {
                const approvers = Array.from(participants.values()).filter((p) => p.info.roles.includes("approver") ||
            ...
            case "role": {
                const roleApprovers = Array.from(participants.values()).filter((p) => p.info.roles.includes(quorum.role));
            ...
            case "specific": {
                const allApproved = quorum.participants.every((id) => gate.approvals.includes(id));
                return {
                    met: allApproved,
                    reason: allApproved
                        ? undefined
    ...
    getEligibleApprovers(quorum, participants) {
        switch (quorum.type) {
            case "any":
            case "all":
            case "majority":
                return Array.from(participants.values())
                    .filter((p) => p.info.roles.includes("approver") ||
                    p.info.capabilities?.includes("approve"))
                    .map((p) => p.info.id);
            case "role":
                return Array.from(participants.values())
