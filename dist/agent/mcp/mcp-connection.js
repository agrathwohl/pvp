...
export class MCPConnection {
    ...
    async connect() {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new Error("Connection timeout")), this.config.startup_timeout_ms ?? 30000);
            });
            ...
            logger.info({ server: this.config.name, toolCount: this.state.tools.length }, "Connected to MCP server");
        }
        catch (error) {
    ...
    async handleConnectionLoss() {
        ...
        if (this.state.reconnect_attempts >= maxAttempts) {
            this.state.status = "error";
            logger.error({ server: this.config.name, attempts: this.state.reconnect_attempts }, "Max reconnection attempts reached");
            return;
        }
        this.state.status = "reconnecting";
        this.state.reconnect_attempts++;
        logger.info({ server: this.config.name, attempt: this.state.reconnect_attempts }, "Attempting to reconnect");
