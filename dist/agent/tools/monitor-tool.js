...
function calculateStopDelay(stopCondition) {
    switch (stopCondition.type) {
        case "duration":
            if (stopCondition.duration_seconds) {
                return Math.min(stopCondition.duration_seconds * 1000, MAX_MONITORING_DURATION);
            }
            return null;
        case "clock_time":
...
async function collectFromFile(source, filePositions, workingDir) {
    if (!source.path) {
...
async function collectFromCommand(source, workingDir) {
    if (!source.command) {
        return { data: "", bytes: 0, is_new: false };
    }
    try {
...
async function collectFromUrl(source) {
    if (!source.url) {
        return { data: "", bytes: 0, is_new: false };
    }
    try {
...
function generateNotebookContent(session, report) {
    const cells = [
        // Title cell
        {
            cell_type: "markdown",
        ...
        // Summary cell
        {
            ...
            source: [
                ...
                `|--------|------|-------------|-------|---------------|\n`,
                ...report.sources_summary.map(s => `| ${s.label} | ${s.type} | ${s.data_points} | ${s.bytes} | ${s.last_activity?.toISOString() || "N/A"} |\n`),
            ],
        },
        // Insights cell
        {
...
// ===========================================================================
export function createMonitorToolHandler() {
    ...
    function checkIdleCondition(session) {
    ...
    async function stopSession(session, reason) {
        ...
        const sourcesSummary = session.config.sources.map((source, index) => {
            const label = getSourceLabel(source, index);
            const dataPoints = session.cycles.flatMap(c => c.data_points.filter(dp => dp.source_label === label));
    ...
    async function scheduleCycle(session) {
        ...
        try {
            ...
            // Schedule next cycle
            session.timer = setTimeout(() => scheduleCycle(session), (session.config.interval_seconds || DEFAULT_INTERVAL_SECONDS) * 1000);
        }
        catch (error) {
            const errMsg = error instanceof Error ? error.message : "Unknown error";
            session.state = "error";
    ...
    return {
        proposeMonitor(input, sessionId, agentId) {
            // Validate input
            ...
            const riskLevel = "high"; // Contains shell execution and HTTP requests
        ...
        async executeMonitor(toolProposalId, input, sessionId, agentId, broadcast, workingDir) {
