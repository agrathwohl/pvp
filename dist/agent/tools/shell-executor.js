...
export function categorizeCommand(command) {
    ...
    const parts = parsed.filter((p) => typeof p === "string");
    const cmd = parts[0] || "";
 ...
 */
export function isCommandBlocked(shellCmd) {
 ...
 */
export async function executeShellCommand(shellCmd, config, callbacks) {
