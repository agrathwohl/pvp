 ...
 */
async function findJupyterExecutable(workingDir) {
 ...
 */
function getOutputPath(notebookPath, format) {
    if (format === "notebook") {
 ...
 */
function createNbconvertCommand(jupyterPath, notebookPath, outputFormat, outputPath, workingDir) {
    const args = [
        "nbconvert",
        "--execute",
        "--to", outputFormat,
 ...
 */
function broadcastError(toolProposalId, sessionId, agentId, broadcast, errorMessage, errorDetail, executionTime) {
...
export function createNotebookToolHandler() {
    return {
        proposeNotebookExecute(notebookPath, outputFormat, sessionId, agentId) {
            ...
            const riskLevel = "high"; // Notebooks execute arbitrary code
        ...
        async executeNotebook(toolProposalId, notebookPath, outputFormat, sessionId, agentId, broadcast, workingDir) {
            ...
            try {
                ...
                const changeMessages = createFileChangeMessages(changes, sessionId, agentId, `notebook: ${path.basename(notebookPath)}`);
                ...
                if (exitCode !== 0) {
                    ...
                    broadcastError(toolProposalId, sessionId, agentId, broadcast, `Notebook execution failed (exit code ${exitCode})`, errorDetail, executionTime);
                    return {
                        success: false,
                ...
                try {
                    ...
                    broadcastError(toolProposalId, sessionId, agentId, broadcast, `Failed to read output file: ${outputPath}`, errorDetail, executionTime);
                    return {
                        success: false,
                        notebookPath: resolvedNotebookPath,
                        outputFormat,
                ...
                broadcastError(toolProposalId, sessionId, agentId, broadcast, `Error: ${errorMsg}`, errorMsg, executionTime);
                return {
                    success: false,
                    notebookPath: resolvedNotebookPath,
                    outputFormat,
